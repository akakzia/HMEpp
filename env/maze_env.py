...
from env import maze_env_utils, maze_task
from env.agent_model import AgentModel
...
class MazeEnv(gym.Env):
    def __init__(
        ...
        **kwargs,
        ...
        self.cells_graph = self._get_cells_graph()
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        # Rescale 
        row = (row - 2) * self._maze_size_scaling
        col = (col - 2) * self._maze_size_scaling
        wrapped_obs['achieved_cell'] = np.array((col, row))
        return wrapped_obs
    ...
    def reset_goal(self, goal: np.ndarray) -> np.ndarray:
        self.t = 0

        # Reset agent starting position
        self.wrapped_env.set_xy(self._init_positions[0])

        # Define GoalRewardMaze task according to input 
        self._task = self.maze_task(scale=self._maze_size_scaling, goal=goal)
        # Reset viewer according to input
        self.wrapped_env.sim.model.site_pos[0] = np.array([goal[0], goal[1], 0.])
    ...
    def set_marker(self) -> None:
        for i, goal in enumerate(self._task.goals):
            ...
            self.wrapped_env.sim.data.site_xpos[idx][: len(goal.pos)] = goal.pos
        self.wrapped_env.sim.forward()
    ...
    def render(self, mode="human", **kwargs) -> Optional[np.ndarray]:
        if mode == "human" and self._websock_port is not None:
            if self._mj_offscreen_viewer is None:
                ...
                from env.websock_viewer import start_server
    ...
    def _get_cells_graph(self) -> List[Tuple[int, int, float, float]]:
        """ Returns list of all cells in the maze [col_index, row_index, x, y] """
        structure = self._maze_structure
        coords = []
        for i, j in it.product(range(len(structure)), range(len(structure[0]))):
            if structure[i][j].is_empty():
                # Ignore the bordure walls
                coords.append((j-1, i-1, (j-1) * self._maze_size_scaling, (i-1) * self._maze_size_scaling))
        return coords
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        outer_reward = self.compute_reward(next_obs['achieved_goal'], next_obs['desired_goal'])
