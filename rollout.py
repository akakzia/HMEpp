...
class RolloutWorker:
    ...
    def current_config(self):
        return tuple(self.last_obs['achieved_cell'])
    ...
    def reset(self):
        ...
        self.current_goal_id = None
        self.last_episode = None
        self.last_obs = self.env.unwrapped.reset_goal(goal=np.array([0, 0]))
        self.dijkstra_to_goal = None
        # Internalization
    ...
    def test_rollout(self,goals,agent_network:SemanticNetwork,episode_duration, animated=False):
        end_episodes = []
        for goal in goals : 
            self.reset()
    ...
    def test_social_rollouts(self,goals,agent_network:SemanticNetwork,episode_duration, animated=False):
        end_episodes = []
        for goal in goals :
            self.reset()
            path, _, _ = agent_network.teacher.oracle_graph.sample_shortest_path(self.last_obs['achieved_cell'], goal,
    ...
    def guided_rollout(self,goal,evaluation,agent_network:SemanticNetwork,episode_duration,episode_budget=None, animated=False):
        episodes = []
        goal = tuple(goal)
    ...
    def generate_one_rollout(self, goal,evaluation, episode_duration, animated=False):
        ...
        ag = self.last_obs['achieved_cell']
        ag_continuous = self.last_obs['achieved_goal']
        ...
        for _ in range(episode_duration):
            ...
            action = self.policy.act(obs.copy(), g.copy(), no_noise)
            ...
            ag_new = observation_new['achieved_cell']
            ag_continuous_new = observation_new['achieved_goal']
            ...
            ep_ag_continuous.append(ag_continuous.copy())

            # Re-assign the observation
            obs = obs_new
            ag = ag_new
            ag_continuous = ag_continuous_new
