...
class Critic(Module):
    def __init__(self, inp_dim, n_quantiles, n_nets):
        super().__init__()
        self.nets = []
        self.n_quantiles = n_quantiles
        self.n_nets = n_nets
        for i in range(n_nets):
            net = Mlp(inp_dim, [256, 256], n_quantiles)
...
class Actor(Module):
    ...
    def select_action(self, obs):
        obs = torch.FloatTensor(obs)[None, :]
...
class TanhNormal(Distribution):
    def __init__(self, normal_mean, normal_std):
        ...
        self.standard_normal = Normal(torch.zeros_like(self.normal_mean),
